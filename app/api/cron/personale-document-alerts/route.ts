...
export async function GET(request: Request) {
  ...
  if (isVercelCronRequest(request) || isAuthorizedCron(request)) {
    try {
      ...
      return NextResponse.json({ ok: false, error: err?.message || "Missing Supabase envs" }, { status: 500 });
    }
  } else {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: err?.message || "Errore caricamento operatori" }, { status: 500 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: err?.message || "Errore cron alert documenti personale" },
      { status: 500 }
    );
  }
}
