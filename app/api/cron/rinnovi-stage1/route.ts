...
export async function GET(request: Request) {
  ...
  const selectWithClienteId =
    "id, cliente, cliente_id, item_tipo, riferimento, descrizione, checklist_id, scadenza, stato, proforma, cod_magazzino, notify_stage1_sent_at, checklists:checklist_id(id, nome_checklist, proforma, magazzino_importazione)";
  const selectWithoutClienteId =
    "id, cliente, item_tipo, riferimento, descrizione, checklist_id, scadenza, stato, proforma, cod_magazzino, notify_stage1_sent_at, checklists:checklist_id(id, nome_checklist, proforma, magazzino_importazione)";
  ...
  for (const [cliente, list] of grouped.entries()) {
    ...
    const messaggio = buildStage1Message(cliente, list);
    const subject = `[Art Tech] Scadenze servizi – ${cliente || "—"}`;
    const html = buildStage1Html(cliente, list);
    try {
    ...
    const { error: updErr } = await supabase
      ...
      .update({
        ...
        notify_stage1_to_operatore_id: recipient?.id ?? null,
      })
      .in("id", ids);
    if (updErr) {
