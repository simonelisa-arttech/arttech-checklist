...
async function selectOperatoriForNotifications(supabase: any, columns: string) {
  const withReparto = await supabase
    .from("operatori")
  ...
  return supabase.from("operatori").select(columns).eq("attivo", true);
}

async function listOperatoriForNotifications(supabase: any): Promise<OperatoreRow[]> {
  const withRiceve = await selectOperatoriForNotifications(supabase, "email, ruolo, attivo, riceve_notifiche");
  ...
  const fallback = await selectOperatoriForNotifications(supabase, "email, ruolo, attivo, alert_enabled");
